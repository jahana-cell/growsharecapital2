...
export default function HomeClientPage({ initialStories }: { initialStories: Story[] }) {
  ...
  return (
    <div className="flex min-h-screen flex-col bg-cream-base text-cream-text font-sans selection:bg-truffle-dark selection:text-cream-base">
        {/* --- 1. HERO SECTION --- */}
            ...
            >
                ...
                <motion.div variants={fadeUp} className="flex flex-col items-center gap-6 md:gap-8 px-4">
                    ...
                    <p className="text-sm md:text-base font-light tracking-wide text-neutral-600 max-w-xl leading-relaxed">
                        <strong className="font-semibold text-truffle-dark uppercase tracking-widest text-[10px] md:text-xs block mb-3">Intelligent. Ethical. High-Yield.</strong>
                        Deploying strategic capital into the essential assets that sustain civilization: <span className="text-truffle-dark font-medium border-b border-neutral-300 pb-0.5">Soil</span>, <span className="text-truffle-dark font-medium border-b border-neutral-300 pb-0.5">Shelter</span>, and <span className="text-truffle-dark font-medium border-b border-neutral-300 pb-0.5">Care</span>.
                    </p>
                </motion.div>
            </motion.div>
            ...
            <div className="max-w-[1600px] mx-auto px-6 md:px-12">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-x-8 gap-y-16">
                    {featuredInvestments.map((item, index) => (
                        ...
                        >
                            <Link href={item.link}>
                                <div className="relative aspect-[3/4] overflow-hidden bg-cream-dark mb-6 md:mb-8">
                                ...
                                <div className="space-y-3 md:space-y-4 pr-4">
                                    ...
                                    <h3 className="text-2xl font-serif text-truffle-dark group-hover:translate-x-2 transition-transform duration-500">
                                        {item.title}
                                    </h3>
                                    <p className="text-sm text-neutral-500 font-light leading-relaxed border-l border-neutral-200 pl-4 group-hover:border-gold-accent transition-colors duration-500">
                                        {item.description}
                                    </p>
                                </div>
                            </Link>
        ...
        <section className="w-full py-24 md:py-32 bg-cream-base">
            <div className="max-w-[1600px] mx-auto px-6 md:px-12">
                <div className="flex flex-col md:flex-row justify-between items-start md:items-baseline mb-16 md:mb-20">
                    ...
                    <Link href="/news" className="group flex items-center gap-2 text-[10px] tracking-[0.3em] uppercase font-bold text-neutral-400 hover:text-gold-accent transition-colors">
                    ...
                    {loading ? (
                        ...
                        <Link key={story.id} href={`/news/${story.slug}`} className="group block h-full">
                            <motion.article 
                                ...
                                viewport={{ once: true }}
                                className="flex flex-col h-full border-t border-neutral-200 pt-6 hover:border-gold-accent transition-colors duration-500"
                            >
                                ...
                                <div className="relative aspect-[16/10] overflow-hidden bg-cream-dark mb-6 w-full md:grayscale md:group-hover:grayscale-0 transition-all duration-700">
                                    <Image
                                        src={story.image}
                                        alt={story.title}
                                        fill
                                ...
                                <h3 className="text-xl md:text-2xl font-serif leading-tight text-truffle-dark group-hover:underline underline-offset-4 decoration-1 decoration-neutral-300">
                                    {story.title}
                                </h3>
                            </motion.article>
                        </Link>
        ...
        <section className="py-32 md:py-40 bg-truffle-dark text-white text-center">
            <div className="container mx-auto px-6">
                ...
                >
                    ...
                    <Link href="/contact" className="inline-block relative group overflow-hidden px-10 py-4 md:px-12 md:py-5 bg-white text-truffle-dark text-[10px] md:text-[11px] tracking-[0.3em] uppercase font-bold">
                        <span className="relative z-10 group-hover:text-white transition-colors duration-500">Inquire Now</span>
                        <div className="absolute inset-0 bg-gold-accent transform translate-y-full group-hover:translate-y-0 transition-transform duration-500 ease-out" />
                    </Link>
                </motion.div>
            </div>
        </section>
